...
summaryWriter directory runname = do
  ...
  pure $ SummaryWriter { runName = name
                       ...
                       , runFilename = "events.haskelltorch." <> T.pack timeUnix <> ".tfevents." <> host
                       , lastStep = step
                       }
...
addHistogram ::
  (TensorConstraints ty 'KCpu sz) =>
  SummaryWriter -> Text -> Tensor ty 'KCpu sz -> IO ()
addHistogram summary name tensor = do
  ...
  withSomeSing (fromIntegral bins) (\ n@(SNat :: SNat m) -> do
    ...
    min   <- toDouble <$> (fromScalar =<< Tensor.min tensor)
    max   <- toDouble <$> (fromScalar =<< Tensor.max tensor)
    let step = (max-min)/2
    let binLimits = V.generate bins (\i -> step * fromIntegral i)
    sum     <- fromScalar =<< Tensor.sum tensor
    sumSq <- fromScalar =<< Tensor.sum =<< Tensor.pow tensor =<< toScalar 2
    writeEvent summary
      (EventSummary name
       (SummaryHistogram
         { shMin = min
         , shMax = max
         , shNum = fromIntegral $ numel tensor
         , shSum = toDouble sum
...
addImageRGB :: (KnownNat szw, KnownNat szh, SingI ty)
            => SummaryWriter -> Text -> Tensor ty 'KCpu '[3, szh, szw] -> IO ()
...
addImageGrey :: (KnownNat szw, KnownNat szh, SingI ty)
             => SummaryWriter -> Text -> Tensor ty 'KCpu '[1, szh, szw] -> IO ()
...
addImageRGBGrid ::
  forall (imagesPerRow :: Nat) (padding :: Nat) (rows :: Nat) (szh :: Nat) (szw :: Nat) (nr :: Nat) (ty :: TensorType).
    _
  => SummaryWriter
  -> Text
  -> Size imagesPerRow
  ...
  -> IO ()
addImageRGBGrid summary name sz padding padValue images = do
  grid <- makeRGBGrid sz padding padValue images
...
addGraph summary name trace = do
  ...
  inputs' <- mapM (\i -> do
                    ...
                    if b then do
                      sz <- C.value_sizes i
                      ty <- C.value_scalar_type i
                      pure (na, Just sz, Just ty)
                      else
                   ...
                   defMessage & #name .~ "input_" <> name
                              ...
                              & #attr .~ M.fromList [("_output_shapes",
                                                      defMessage & #list .~
                                                       (defMessage & #shape .~
                                                         [defMessage & #dim .~
                                                          map (\x -> defMessage & #size .~ x) (V.toList sz)]))]
                 (_, _) ->
                   defMessage & #name .~ "input_" <> name
                              & #op .~ "Input")
         inputs'
...
-- tensor.
addTensorSummary :: (Num (TensorTyToHs ty), Num (TensorTyToHsC ty),
                     V.Storable (TensorTyToHs ty), V.Storable (TensorTyToHsC ty),
                     SingI sz, SingI ty) =>
                   SummaryWriter -> Text -> Tensor ty 'KCpu sz -> IO ()
addTensorSummary summary name tensor = do
  mean   <- fromScalar =<< Tensor.mean tensor
  median <- fromScalar =<< Tensor.median tensor
  var    <- fromScalar =<< Tensor.var tensor True
  min    <- fromScalar =<< Tensor.min tensor
  max    <- fromScalar =<< Tensor.max tensor
