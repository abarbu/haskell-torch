...
data Model = Model { w1 :: LSTMParams TFloat KBest 28 128 2 False True,
                     w2 :: LinearParam TFloat KBest 128 10 }
...
forward (Model{..},state1) isTraining =
           lstmBatchFirst (inF_ @28) (hiddenF_ @128) (nrLayers_ @2) (isBidirectional_ @False) 0 isTraining w1 state1
        >=> (\(a,s) -> select @1 a (-1))
        >=> linear (inF_ @128) (outFeatures_ @10) w2
...
ex = do
  ...
  optimizer <- newIORef (adam (def { adamLearningRate = 0.01 }) params)
  --
